...
export class Cell extends Phaser.Physics.Arcade.Sprite {
  ...
  handleColisionCellWithPlayer() {
    ...
    // quitar vida al cell
    this.scene.cell.takeDamage(10);
  }

  takeDamage(amount) {
    if (this.alive) {
      this.scene.uiController.enemyHealthBar.setHealth(
        this.scene.uiController.enemyHealthBar.currentHealth - amount
      );
      if (this.scene.uiController.enemyHealthBar.currentHealth <= 0) {
        this.die();
      }
    }
  }
